...
import { CalendarClock, Check, CheckCheck, Loader2, Send } from 'lucide-react';
 ...
 *
 * <p><b>A fila é alimentada por máquina e esvaziada por pessoa</b>, e esse desequilíbrio já
 * cobrou caro: em 06/09/2026 ela amanheceu com 1.378 linhas das quais ~34 eram reais. As causas
 * foram corrigidas no motor; daqui ficou o tratamento em lote, porque uma linha por clique não
 * sobrevive a um dia ruim do SISREG.</p>
 */
export function AlteracoesAgendaPage() {
  ...
  const tratarLote = useTratarLote();

  const itens = useMemo(() => lista.data?.itens ?? [], [lista.data]);
  const pendentes = useMemo(() => itens.filter((a) => !a.tratadaEm), [itens]);

  // Só conta o que ainda está na tela: uma linha tratada por outro operador enquanto esta aba
  // estava aberta não pode continuar contando no botão.
  const marcadas = useMemo(
    () => pendentes.filter((a) => selecionadas.has(a.id)).map((a) => a.id),
    [pendentes, selecionadas],
  );

  const todasMarcadas = pendentes.length > 0 && marcadas.length === pendentes.length;
  ...
  function alternarTodas() {
    setSelecionadas(todasMarcadas ? new Set() : new Set(pendentes.map((a) => a.id)));
  }
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 max-w-3xl text-sm text-gray-600">
          ...
          <strong>Sumiu do SISREG</strong> é provável cancelamento lá, e não se avisa paciente a
          partir dele: a mensagem seria uma confirmação do horário antigo. Confirme no SISREG e, se
          caiu mesmo, cancele a solicitação — enquanto estiver aqui a vaga continua contando como
          ocupada.
        </p>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-2 text-sm text-gray-700">
          <input
            ...
            onChange={(e) => {
              setApenasPendentes(e.target.checked);
        ...
        {marcadas.length > 0 ? (
          <Button
            tamanho="sm"
            variante="ghost"
            disabled={tratarLote.isPending}
            title="Marca as selecionadas como resolvidas, sem enviar mensagem a ninguém."
            onClick={agirEmLote}
          >
            {tratarLote.isPending ? (
              <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />
            ) : (
              <CheckCheck className="mr-1.5 h-3.5 w-3.5" />
            )}
            Tratar {marcadas.length} selecionada{marcadas.length > 1 ? 's' : ''}
          </Button>
        ) : null}
      </div>
                ...
                <th className="px-3 py-2 font-medium">
                  {pendentes.length > 0 ? (
                    <input
                      ...
                      title="Selecionar todas as pendentes desta página"
                    />
                  ) : null}
                </th>
                  ...
                  <td className="px-4 py-2">
                    {a.tratadaEm ? (
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {/* Ausente não tem "avisar": a mensagem seria a confirmação de um horário
                            que provavelmente não existe mais. O backend recusa de qualquer forma —
                            aqui o botão some para o operador não chegar a tentar. */}
                        {a.tipo === 'Ausente' ? null : (
                          <Button
                            tamanho="sm"
                        ...
                        <Button
                          ...
                          title={
                            a.tipo === 'Ausente'
                              ? 'Confira no SISREG antes. Se foi cancelado lá, cancele a solicitação; se continua de pé, marque como resolvida aqui.'
                              : 'Marca como resolvida sem enviar mensagem.'
                          }
