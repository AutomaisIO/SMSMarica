...
import { diaCurto, mesCurto, numero } from '../lib/formato';
import type { DiaSemana, SerieDia, SerieMes, TopItem } from '../types';
...
/** Barras de autorizações por dia com a linha de pessoas ativas. */
export function GraficoPorDia({ dados, mostrarOperadores = true }: { dados: SerieDia[]; mostrarOperadores?: boolean }) {
  return (
    <ResponsiveContainer width="100%" height={260}>
      <ComposedChart
        data={dados.map((d) => ({ ...d, rotulo: diaCurto(d.dia) }))}
      ...
      >
        ...
        <Bar yAxisId="a" dataKey="autorizacoes" name="Autorizações" fill="#93c5fd" radius={[3, 3, 0, 0]} />
        {mostrarOperadores ? (
          <Line
            yAxisId="o"
            type="monotone"
...
export function GraficoPorMes({ dados, mostrarOperadores = true }: { dados: SerieMes[]; mostrarOperadores?: boolean }) {
  return (
    <ResponsiveContainer width="100%" height={260}>
      <ComposedChart
        data={dados.map((m) => ({ ...m, rotulo: mesCurto(m.mes) }))}
        margin={{ top: 4, right: 8, bottom: 8, left: 4 }}
      >
        <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
        ...
        {mostrarOperadores ? <YAxis yAxisId="o" orientation="right" tick={{ fontSize: 11 }} allowDecimals={false} /> : null}
        <Tooltip contentStyle={{ fontSize: 12 }} />
        <Bar yAxisId="a" dataKey="autorizacoes" name="Autorizações" fill="#60a5fa" radius={[4, 4, 0, 0]} />
        {mostrarOperadores ? (
          <Line
            yAxisId="o"
            type="monotone"
...
export function GraficoDiaSemana({ dados }: { dados: DiaSemana[] }) {
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={dados} margin={{ top: 4, right: 8, bottom: 8, left: 4 }}>
        ...
        <Bar dataKey="autorizacoes" name="Total" fill="#d1d5db" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
...
export function ListaTop({ itens, vazio = 'Nada no período.' }: { itens: TopItem[]; vazio?: string }) {
  const max = Math.max(1, ...itens.map((i) => i.autorizacoes));
        ...
        <li key={i.rotulo} className="flex items-center gap-2 text-xs">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="shrink-0 tabular-nums text-gray-600">{numero(i.autorizacoes)}</span>
            </div>
            <div className="mt-0.5 h-1.5 overflow-hidden rounded-full bg-gray-100">
              <div className="h-full bg-primary-400" style={{ width: `${Math.round((i.autorizacoes / max) * 100)}%` }} />
            </div>
          </div>
        </li>
      ))}
