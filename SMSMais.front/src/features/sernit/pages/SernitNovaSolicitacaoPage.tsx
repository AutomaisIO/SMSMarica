...
import {
  useAnexarRascunhoSernit,
  useCamposCatalogoSernit,
  useFormularioCatalogoSernit,
  useMarcarRascunhoProntoSernit,
  useRascunhoSernit,
  useRascunhosSernit,
  useRemoverAnexoRascunhoSernit,
  useSalvarRascunhoSernit,
} from '@/features/sernit/api/queries';
import type {
  CampoDinamicoSernit,
  CatalogoRecursoSernit,
  OpcaoSernit,
  RascunhoSernitLista,
...
import { pesquisarPacienteSernit } from '@/features/sernit/api/sernitApi';

/**
 * Regulação → Nova solicitação.
 *
 * <p>O formulário é montado do NOSSO catálogo espelhado — offline e instantâneo. O pedido é
 * guardado aqui como rascunho, com anexos, e só vai ao SERNIT quando o envio for autorizado.</p>
 *
 * <p>Os campos do bloco dinâmico mudam por recurso: cada recurso produz o seu formulário. A tela
 * desenha o que o catálogo disser que aquele recurso exige.</p>
 */

const CLASSE_STATUS: Record<StatusRascunhoSernit, string> = {
...
export function SernitNovaSolicitacaoPage() {
  const [rascunhoId, setRascunhoId] = useState<string | null>(null);
  const [tipo, setTipo] = useState<TipoRecursoSernit | ''>('');
  const [recurso, setRecurso] = useState<CatalogoRecursoSernit | null>(null);
  const [cns, setCns] = useState('');
  const [paciente, setPaciente] = useState('');
  const [hipotese, setHipotese] = useState('');
  const [campos, setCampos] = useState<Record<string, string>>({});
  const [pacienteSernit, setPacienteSernit] = useState<PacienteEncontradoSernit | null>(null);
  const [buscando, setBuscando] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [aviso, setAviso] = useState<string | null>(null);

  const catalogo = useFormularioCatalogoSernit();
  const dinamicos = useCamposCatalogoSernit(tipo || undefined, recurso?.valor);
  const lista = useRascunhosSernit();
  const carregado = useRascunhoSernit(rascunhoId);

  const salvar = useSalvarRascunhoSernit();
  const marcarPronto = useMarcarRascunhoProntoSernit();
  const anexar = useAnexarRascunhoSernit();
  const removerAnexo = useRemoverAnexoRascunhoSernit();
  ...
  // Filtra pelo tipo escolhido: Consulta e Exame convivem na mesma lista do catálogo.
  const recursosDoTipo = useMemo(
    () => (!tipo ? [] : (catalogo.data?.recursos ?? []).filter((r) => r.tipo === tipo)),
    [catalogo.data, tipo],
  );
  ...
  function corpo() {
    return {
      tipo: (tipo || undefined) as TipoRecursoSernit | undefined,
  ...
  /**
   * Busca o paciente no motor do PRÓPRIO SERNIT (CNS ou CPF) e traz o cadastro.
   *
   * O SERNIT VENCE: o que ele devolver sobrescreve o que estiver preenchido. É a fonte que o pedido
   * vai enfrentar na hora de gravar, então divergir dela só adiaria a recusa.
   *
   * A identidade volta travada (o SERNIT a marca `disabled` e nem a recebe de volta); endereço,
   * nome social e telefones voltam editáveis, que é o que muda na vida do cidadão.
   */
  async function buscarNoSernit() {
    ...
    setBuscando(true);
    try {
      const r = await pesquisarPacienteSernit(doc);
      setPacienteSernit(r);
      const doSernit: Record<string, string> = {};
      for (const c of r.campos) if (c.valor) doSernit[c.campo] = c.valor;
      setCampos({ ...campos, ...doSernit });

      const nome = r.campos.find((c) => c.campo.endsWith(':nome'))?.valor;
      if (nome) setPaciente(nome);
      setAviso(r.encontrado ? 'Cadastro trazido do SERNIT.' : null);
    } catch (e) {
  ...
  async function aoSalvar() {
    ...
    try {
      ...
      setRascunhoId(r.id);
      setAviso('Rascunho salvo. Ele fica guardado aqui até você mandar para o SERNIT.');
    } catch (e) {
      setErro(extrairMensagemDeErro(e));
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {semCatalogo && !catalogo.isLoading && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
          ...
          <p>
            ...
            nem os campos de cada um. Vá em <em>Regulação → Configuração → SERNIT</em> e rode a cópia
      ...
      <div className="flex items-start gap-2 rounded-lg border border-slate-300 bg-slate-50 p-3 text-sm text-slate-700">
        ...
        <p>
          <strong>Envio ao SERNIT ainda desligado.</strong> Tudo aqui é guardado na nossa base —
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_20rem]">
        <div className="space-y-5 rounded-lg border border-slate-200 bg-white p-4">
          {carregado.data && (
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              {carregado.data.idSernitGerado && (
                <span className="text-slate-600">nº no SERNIT: {carregado.data.idSernitGerado}</span>
              )}
            ...
            <div className="flex flex-wrap items-end gap-3">
              <Campo label="Tipo *" htmlFor="ns-tipo" className="w-44">
                <Select
                  ...
                  onChange={(e) => {
                    setTipo(e.target.value as TipoRecursoSernit | '');
              ...
              <Campo label="Recurso *" htmlFor="ns-recurso" className="min-w-96 flex-1">
                <SeletorRecursoSernit
                  ...
                  onChange={(r) => {
                    ...
                    // no oncológico, e o SERNIT recusaria o pedido sem dizer por quê.
                    setHipotese('');
                  }}
                />
              </Campo>
          ...
          <section>
            ...
            <div className="flex flex-wrap items-end gap-3">
              {/* O SERNIT aceita CNS ou CPF no mesmo campo — a tela dele decide pelo tamanho. */}
              <Campo label="CNS ou CPF *" htmlFor="ns-cns" className="w-56">
                <Input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); buscarNoSernit(); } }}
                />
              </Campo>
              <Button variante="secundaria" onClick={buscarNoSernit} disabled={somenteLeitura || buscando}>
                {buscando ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}
                Buscar no SERNIT
              </Button>
              {pacienteSernit && !pacienteSernit.encontrado && (
                <span className="text-sm text-amber-700">
                  O SERNIT não encontrou ninguém com esse número.
                </span>
              )}
            </div>

            {/* Avisos do SERNIT — inclui "o CNS definitivo é diferente do provisório", que é a
                armadilha de identidade mais cara que conhecemos. */}
            {pacienteSernit?.avisos.map((a) => (
              <p key={a} className="mt-2 rounded bg-amber-50 p-2 text-xs text-amber-900">{a}</p>
            ))}

            {pacienteSernit?.encontrado && (
              <div className="mt-3 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                {pacienteSernit.campos.map((c) => (
                  <CampoPaciente
                    key={c.campo}
                    c={c}
                    ...
                    /* Sugestão só no campo de WhatsApp, e amarrada pelo RÓTULO: dois dos três
                       telefones do SERNIT têm id posicional (j_idNNN), que muda quando Niterói
                       recompila. */
                    sugestao={
                      /whatsapp/i.test(c.rotulo) ? pacienteSernit.telefoneVerificadoNosso : null
                    }
                  />
                ))}
              </div>
          ...
          <section>
            ...
            <div className="flex flex-wrap items-end gap-3">
              <Campo label="Classificação de risco *" htmlFor="ns-risco" className="w-56">
                ...
                >
                  <option value="">Selecione…</option>
                  {(catalogo.data?.classificacoesRisco ?? []).map((o: OpcaoSernit) => (
              ...
              <Campo label="Médico responsável" htmlFor="ns-medico" className="min-w-72 flex-1">
                ...
                >
                  <option value="">Selecione…</option>
                  {(catalogo.data?.medicos ?? []).map((o: OpcaoSernit) => (
                    <option key={o.valor} value={o.valor}>{o.rotulo}</option>
                  ))}
                </Select>
              </Campo>

              {/* Não é campo de texto: no SERNIT a Hipótese é a caixa de CID, e a relação de
                  códigos é a DAQUELE recurso — por isso o seletor recebe o recurso. */}
          ...
          <section>
            ...
            {recurso && !dinamicos.isLoading && listaCampos.length === 0 && (
              <p className="text-sm text-amber-700">
                Este recurso ainda não teve os campos copiados do SERNIT.
              </p>
            )}
            ...
            <ul className="mb-2 space-y-1">
              {(carregado.data?.anexos ?? []).map((a) => (
                <li key={a.id} className="flex items-center gap-2 text-sm">
                  ...
                  {a.enviadoEm ? (
                    <span className="shrink-0 text-xs text-emerald-700">no SERNIT</span>
                  ) : (
                    !somenteLeitura && (
                      <button
                        type="button"
          ...
          {!somenteLeitura && (
            <section className="flex flex-wrap items-center gap-2 border-t border-slate-200 pt-4">
              ...
              <Button variante="secundaria" disabled title="O envio ao SERNIT ainda não está habilitado">
                <Send className="size-4" />
                Enviar ao SERNIT (desligado)
              </Button>
            </section>
          )}
        </div>
        ...
        <aside className="space-y-2">
          ...
          {(lista.data ?? []).map((r: RascunhoSernitLista) => (
            <button
              key={r.id}
              type="button"
...
/**
 * Desenha um campo do cadastro que o SERNIT devolveu.
 *
 * Travado quando o SERNIT o marcou `disabled` — nome, CPF, CNS, nascimento, sexo, mãe e raça. Não é
 * capricho de tela: campo `disabled` não é enviado pelo navegador, então esses valores nem chegam
 * ao Gravar. Deixá-los editáveis prometeria uma correção que o SERNIT descarta em silêncio.
 */
function CampoPaciente({
  ...
  const mesmosDigitos = (a: string, b: string) => {
    ...
    // Tolera DDI: um é sufixo do outro (o verificado guarda "55…", o SERNIT guarda nacional).
...
function CampoDinamico({
  ...
  // Múltipla escolha. O SERNIT recebe o MESMO nome repetido, um par por opção marcada; no rascunho
  // isso cabe num par nome→valor porque os valores viajam juntos separados por quebra de linha —
  // o contrato de SernitValorMultiplo no backend, que desdobra na hora do envio.
  if (c.tipo === 'checkbox' && c.opcoes?.length) {
    const marcadas = new Set(valor ? valor.split('\n').filter(Boolean) : []);
    const alternar = (v: string) => {
      if (marcadas.has(v)) marcadas.delete(v);
      else marcadas.add(v);
      // Reordena pela ordem do SERNIT, não pela ordem dos cliques.
  ...
  // virar caixa de texto em silêncio: o SERNIT só aceita os valores da lista dele, e o pedido
    ...
    return (
      <Campo label={rotulo} htmlFor={id} className="min-w-72">
        ...
        <p className="mt-1 text-xs text-amber-700">
          Sem as opções deste campo. Recopie o catálogo do SERNIT em Configurações para liberá-lo.
        </p>
      </Campo>
    );
  }
  ...
  // O SERNIT usa dd/MM/yyyy no rich:calendar. O input nativo de data fala ISO, então a conversão
  // acontece na borda — o que sai daqui para o payload é sempre o formato que o SERNIT espera.
  if (c.tipo === 'date') {
    return (
      <Campo label={rotulo} htmlFor={id} className="w-52">
        <Input
...
export default SernitNovaSolicitacaoPage;
