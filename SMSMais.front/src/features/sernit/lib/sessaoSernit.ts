...
import { useSessaoOperadorSernit } from '@/features/sernit/api/queries';
import { useAuth } from '@/shared/auth/authStore';
import type { ProblemaApi } from '@/shared/api/httpClient';

/** Código que o backend devolve quando não há sessão de escrita no SERNIT. */
export const SEM_SESSAO_SERNIT = 'sernit.sessao_operador_ausente';
...
/**
 * Garante a sessão de escrita no SERNIT antes de uma ação — pedindo a senha, nunca mostrando erro.
 *
 * <p><b>Por que existe:</b> a recusa do backend ("é preciso entrar com o SEU usuário do SERNIT…")
 ...
 */
export function useSessaoSernitObrigatoria() {
  const { data: sessao } = useSessaoOperadorSernit();
  // Quem opera é o usuário do SMSMais; o login do SERNIT é credencial, não identificação de
  ...
  const tratouFaltaDeSessao = useCallback((erro: unknown, repetir: () => void | Promise<void>) => {
    if (!temCodigo(erro, SEM_SESSAO_SERNIT)) return false;
  ...
  return {
    ...
    operador: nomeDoOperador ?? sessao?.usuarioSernit ?? null,
    usuarioSernit: sessao?.usuarioSernit ?? null,
    comSessao,
    tratouFaltaDeSessao,
    /** Props prontas para o `<ModalLoginSernit />`. */
