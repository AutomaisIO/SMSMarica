...
import {
  useDispararVarreduraSernit,
  useFormularioCatalogoSernit,
  useSincronizarCatalogoSernit,
  useSalvarVarreduraAutomaticaSernit,
  useVarreduraAutomaticaSernit,
  useExecucoesSernit,
  useSalvarCredencialSernit,
  useStatusMotorSernit,
  useTestarCredencialSernit,
} from '@/features/sernit/api/queries';
import type { ExecucaoSernit, ModoVarreduraSernit, StatusVarreduraSernit } from '@/features/sernit/types';

const CLASSE_STATUS: Record<StatusVarreduraSernit, string> = {
...
const ROTULO_MODO: Record<ModoVarreduraSernit, string> = {
 ...
 */
function descreverFase(x: ExecucaoSernit): string {
  switch (x.fase) {
    case 'Grade':
...
function desdeUltimoSinal(iso: string | null): { texto: string; velho: boolean } | null {
  ...
  // 2 min sem gravar nada é muito: o lote mais lento medido contra o SERNIT leva ~5s.
...
export function AbaSernitConfiguracao() {
  const { data: status, isLoading } = useStatusMotorSernit();
  // A lista segue o mesmo ritmo do status: é nela que ficam fase, cursor e pendentes da rodada.
  const { data: execucoes } = useExecucoesSernit(20, status?.varreduraEmAndamento ?? false);
  const disparar = useDispararVarreduraSernit();
  const { data: automatica } = useVarreduraAutomaticaSernit();
  const { data: catalogo } = useFormularioCatalogoSernit();
  const sincronizarCatalogo = useSincronizarCatalogoSernit();
  const salvarAutomatica = useSalvarVarreduraAutomaticaSernit();
  const testar = useTestarCredencialSernit();
  const salvar = useSalvarCredencialSernit();

  const [modo, setModo] = useState<ModoVarreduraSernit>('Diaria');
  ...
  async function aoTestar() {
    ...
    try {
      ...
      setAviso('Credencial aceita pelo SERNIT e módulo Ambulatório acessível.');
    } catch (e) {
      setErro(extrairMensagemDeErro(e));
    }
  }
  ...
  async function aoSalvar() {
    ...
    try {
      ...
      setSenha('');
      setAviso('Credencial validada no SERNIT e salva (cifrada). O motor já pode rodar.');
    } catch (e) {
      setErro(extrairMensagemDeErro(e));
    }
  }

  const colunas: Coluna<ExecucaoSernit>[] = [
    {
      chave: 'iniciadoEm',
      cabecalho: 'Início',
      className: 'w-40',
    ...
    <div className="space-y-4">
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              ...
              <div className="text-xs text-slate-500">Credencial do SERNIT</div>
            </div>
          </div>

          {!status.credencialConfigurada && (
            <p className="rounded bg-amber-50 p-3 text-sm text-amber-800">
              A credencial do SERNIT ainda não está cadastrada. Ela é gravada na tela de
              <strong> Integrações</strong>, no provedor <code>sernit</code>. Use o teste abaixo para
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Campo label="Modo da rodada" htmlFor="sernit-modo-da-rodada" className="min-w-80 flex-1">
            <Select id="sernit-modo-da-rodada" value={modo} onChange={(e) => setModo(e.target.value as ModoVarreduraSernit)}>
              {(Object.keys(ROTULO_MODO) as ModoVarreduraSernit[]).map((m) => (
        ...
        {/* Cópia do catálogo: é o que deixa a tela de nova solicitação OFFLINE. Leitura longa
            (uma ida ao SERNIT por recurso) e retomável — o botão avisa o custo. */}
        <div className="mt-4 flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="min-w-64 flex-1 text-sm text-slate-700">
            <strong>Catálogo do SERNIT</strong>{' '}
        ...
        <div className="mt-4 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <Campo label="Horário (Brasília)" htmlFor="sernit-hora" className="w-40">
            <Input
              id="sernit-hora"
              type="time"
              value={hora}
        ...
        <p className="mt-3 text-xs text-slate-500">
          ...
          roda em segundo plano. Não é possível filtrar por “alterado desde” — o SERNIT não tem esse
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold">
          <KeyRound className="size-4 text-red-600" /> Credencial do SERNIT
        </h3>
        <div className="flex flex-wrap items-end gap-3">
          <Campo label="Usuário" htmlFor="sernit-usuario" className="w-56">
            <Input id="sernit-usuario" value={usuario} onChange={(e) => setUsuario(e.target.value)} autoComplete="off" />
          </Campo>
          <Campo label="Senha" htmlFor="sernit-senha" className="w-56">
            <Input
              id="sernit-senha"
              type="password"
              value={senha}
