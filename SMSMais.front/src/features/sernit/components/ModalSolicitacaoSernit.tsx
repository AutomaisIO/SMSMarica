...
/**
 * Detalhe completo da solicitação do SERNIT, em modal.
 *
 ...
 */
export function ModalSolicitacaoSernit({
  solicitacaoId,
  aoFechar,
}: {
  ...
  const { data, isLoading } = useSolicitacaoSernit(solicitacaoId ?? undefined);
  const r = data?.resumo;
  ...
  return (
    <Modal
      ...
      aoFechar={aoFechar}
      largura="lg"
      titulo={r ? `Solicitação ${r.idSernit}` : 'Solicitação'}
        ...
        <div className="space-y-5 text-sm">
          ...
          {solicitacaoId && (
            <section className="flex flex-wrap items-center gap-2">
              <PainelFollowUpSernit solicitacaoId={solicitacaoId} />
              <PainelContatosSernit solicitacaoId={solicitacaoId} />
            </section>
          )}
