...
import { useEntrarNoSernit } from '@/features/sernit/api/queries';
...
/**
 * Entrada no SERNIT com a credencial do PRÓPRIO operador.
 *
 * <p>Por que não basta a credencial já cadastrada em Regulação → Configuração: aquela é de
 * <b>sincronismo</b> — serve para a varredura ler a fila. O SERNIT carimba cada evento com o nome de
 * quem fez (medido em 18/08/2026), então escrever com ela faria toda ação do município aparecer
 * assinada pela mesma pessoa na trilha de auditoria de Niterói.</p>
 *
 * <p>A senha vai para a memória do servidor amarrada à sessão do usuário e <b>não é gravada em
 * banco</b>. Sair do sistema derruba também a sessão do SERNIT.</p>
 *
 * <p><b>O OK valida na hora.</b> O backend tenta logar no SERNIT de verdade antes de guardar
 * qualquer coisa: se a credencial não abrir o SERNIT, o modal continua aberto com o
 ...
 */
export function ModalLoginSernit({
  aberto,
  aoFechar,
  aoAutenticar,
}: {
  ...
  const entrar = useEntrarNoSernit();
  ...
  async function aoEnviar(e: React.FormEvent) {
    ...
    try {
      ...
      // porque é esse nome que vai para o histórico da solicitação no SERNIT.
      setOkComo(info.usuarioSernit ?? usuario.trim());
      ...
      {okComo ? (
        <div className="space-y-3">
          <p className="flex items-center gap-2 rounded border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
            ...
            <span>
              Autenticado no SERNIT como <strong>{okComo}</strong>.
            </span>
          </p>
        ...
        <form onSubmit={aoEnviar} className="space-y-4">
          ...
          <label className="block space-y-1">
            <span className="text-sm text-slate-700">Usuário do SERNIT</span>
            <Input
              value={usuario}
          ...
          <label className="block space-y-1">
            <span className="text-sm text-slate-700">Senha do SERNIT</span>
            <Input
              type="password"
              value={senha}
