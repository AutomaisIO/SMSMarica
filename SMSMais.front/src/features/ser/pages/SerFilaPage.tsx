...
export function SerFilaPage() {
  ...
  const filtro = useMemo<BuscaSerFiltro>(
    () => ({
  ...
  const { data: resultado, isLoading } = useBuscaSer(filtro);
  const { data: resumo } = useResumoSer();
  ...
  function trocarSituacao(nova: SituacaoSer | '') {
    setSituacao(nova);
    setPagina(1);
  }

  const colunas: Coluna<SolicitacaoSerLista>[] = [
    {
      chave: 'idSer',
      cabecalho: 'ID',
      className: 'w-24',
      ordenar: (s) => Number(s.idSer),
      render: (s) => <span className="font-mono text-xs text-slate-600">{s.idSer}</span>,
    },
    {
      chave: 'tipo',
      cabecalho: 'Tipo',
    ...
    },
    {
      ...
      render: (s) => (
        <div className="space-y-1">
          <SituacaoSerBadge situacao={s.situacao} />
    ...
    },
    {
      ...
      render: (s) =>
        s.historicoIndisponivel ? (
          // O SER não oferece histórico para solicitações em Alta — explicar é melhor
          // que mostrar "0 eventos" e deixar o operador achar que perdemos o dado.
          <span className="text-xs text-slate-400" title="O SER não oferece histórico nesta situação">
            indisponível
          </span>
        ) : (
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold">SER — fila do Estado</h1>
          <p className="text-sm text-slate-500">
            Solicitações de consulta e exame reguladas pela SES-RJ, espelhadas na nossa base.
            A busca lê o nosso banco — o motor de varredura é quem fala com o SER.
          </p>
        </div>
      </header>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4">
        <Campo label="Situação" htmlFor="ser-situacao" className="w-56">
          <Select
            id="ser-situacao"
            value={situacao}
            onChange={(e) => trocarSituacao(e.target.value as SituacaoSer | '')}
          >
        ...
        <Campo label="Buscar" htmlFor="ser-buscar" className="min-w-72 flex-1">
          <Input
            id="ser-buscar"
            value={termo}
            onChange={(e) => setTermo(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && aplicarBusca()}
            placeholder="Nome, CPF, CNS, ID do SER ou recurso"
          />
        </Campo>
      ...
      <Tabela
        ...
        scrollXFlutuante
        aoClicarLinha={(s) => navegar(`/app/regulacao/ser/${s.id}`)}
