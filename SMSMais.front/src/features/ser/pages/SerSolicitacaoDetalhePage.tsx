...
function EventoItem({ evento }: { evento: EventoSer }) {
  return (
...
export function SerSolicitacaoDetalhePage() {
  const { id } = useParams<{ id: string }>();
  const navegar = useNavigate();
  const { data: detalhe, isLoading } = useSolicitacaoSer(id);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Button variante="secundaria" onClick={() => navegar('/app/regulacao/ser')}>
          <ArrowLeft className="size-4" /> Voltar
        </Button>
        <div className="flex-1">
          <h1 className="text-xl font-semibold">
            Solicitação <span className="font-mono">{r.idSer}</span>
          </h1>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <section className="space-y-4 lg:col-span-2">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            {/* Em Alta o SER esconde o histórico — mas NÓS mantemos o que capturamos e anexamos a
                linha da mudança para Alta. Por isso o aviso é um banner ACIMA da trilha, não no
            ...
            {r.historicoIndisponivel && (
              <p className="mb-3 rounded bg-amber-50 p-3 text-sm text-amber-800">
                Em Alta o SER não oferece mais o histórico. Mantivemos o que foi capturado antes e
