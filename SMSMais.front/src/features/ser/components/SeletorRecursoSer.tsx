...
import type { CatalogoRecursoSer } from '@/features/ser/types';

/**
 * Seletor de recurso com busca por digitação.
 *
 * <p>O SER tem 120 recursos em CONSULTA e 83 em EXAME, com nomes longos e cheios de subdivisão
 * ("Ambulatório 1ª vez em Cardiologia - Cardiopatia Congênita (Adulto)"). Um `select` comum com
 * essa lista é inutilizável — a própria tela do SER oferece um campo de digitação ao lado do
 * combo pelo mesmo motivo.</p>
 *
 * <p>Aqui a busca é <b>local</b>, contra o catálogo espelhado: filtra enquanto se digita, sem ida
 * ao servidor. No SER, cada tecla é uma requisição.</p>
 */
export function SeletorRecursoSer({
  recursos,
  valor,
  onChange,
  desabilitado,
  ...
  valor: string;
  onChange: (r: CatalogoRecursoSer | null) => void;
  ...
  function escolher(r: CatalogoRecursoSer) {
    onChange(r);
    setAberto(false);
    setTermo('');
  }
