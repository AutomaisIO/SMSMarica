...
/// efetiva é <c>Σ(linha do quadro: dias acesos × atendimentos/turno) × aproveitamento</c> mais os
...
public static class SimuladorFila
{
    ...
    /// <summary>
    /// Quantos turnos por semana a mais (com <paramref name="atendimentosPorTurno"/> cada, no
    /// aproveitamento vigente) fariam a capacidade chegar a <paramref name="capacidadeAlvo"/>.
    /// Serve para a tela sugerir "faltam 3 turnos" sem chamar o agente.
    /// </summary>
    public static double? TurnosAMaisPara(ParametrosEstrategia p, double capacidadeAlvo, double atendimentosPorTurno)
    {
        var porTurno = Math.Max(0, atendimentosPorTurno) * Math.Clamp(p.Aproveitamento.Valor, 0, 1);
        if (porTurno <= 0) return null;
        return Arredondar(Math.Max(0, capacidadeAlvo - p.CapacidadeSemanal()) / porTurno);
    }
