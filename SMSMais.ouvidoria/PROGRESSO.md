...
| 20/09/2026 | Commit + push | feito | Bernardo autorizou prod. Commits `7bb7242` (módulo) e `3ee933a` (migration, regenerada sobre o HEAD com id mantido `20260920142356`). Chegaram ao origin pelo push de outra sessão no branch compartilhado; `55323b3` (dela) commitou a pasta do manual, que levou o artigo `conteudo/ouvidoria.tsx`. O deploy do servidor só passou em `ef2a6a4` (16:22, após a outra frente corrigir o teste do robô); front e server em PROD nas instâncias `marica` e `ccdti`. Verificado pela API de Maricá: 36 rotas `ouvidoria` no OpenAPI, `GET /publico/ouvidoria/assuntos` = 22 assuntos (tabelas criadas e semeadas), 51 unidades. **Pendente humano:** sair e entrar de novo no painel (permissões são persistidas no login) e marcar os módulos 71–74 nos perfis que não sejam o Administrador |
