...
9. **Uma instância por município — sem `TenantId`, sem rename** — [ADR-0043](./docs/adr/0043-instancia-por-municipio.md). O produto é entregue como **uma instância por prefeitura** (droplet + banco + domínio próprios); o isolamento é físico. Consequências: (a) **não** introduzir `TenantId` em entidade nenhuma — `X-Unidade-Id` continua sendo escopo *entre unidades de um mesmo município* e não ganha segundo nível; (b) `SMSMarica` (namespace/assembly) e `smsmarica` (schema) **não são renomeados** — são o nome de código do produto; (c) **nada institucional em código ou migration**: nome de secretaria, marca, cores, domínios, CNES, número de WhatsApp e contatos legais vivem em `smsmarica.instituicao` (singleton, lido por `IInstituicaoService`, exposto em `GET /publico/instituicao`) ou em variável de ambiente. **Seed institucional nunca entra em migration** — migration é imutável e roda igual em toda instância nova.
