...
| [`docs/adr/`](./docs/adr/) | Decisões arquiteturais registradas (0001 schema, 0003 Android-only, 0004 três projetos, 0005 usuário unificado, 0006 papel derivado, **0007 schema FHIR separado**, **0008 mapeamento clínico FHIR**, **0009 identidade e proveniência multi-PEP**, **0010 serviço FHIR autônomo (Automais.Fhir)**, **0011 módulo IA (consulta em linguagem natural multi-alvo)**, **0012 agendamento local + integração SISREG só-leitura**, **0013 agenda multi-recurso (especialidade/médico/equipamento)**, **0014 importação de PEPs no backend (canal Oracle persistente, multi-base)**, **0036 login do cidadão exige contato verificado**, **0039 unidade de saúde é o eixo durável (Organization + CNES como ponte entre PEPs)**, **0041 paciente sem CPF entra marcado, não fica de fora**, **0043 uma instância por município (whitelabel por configuração, sem tenant compartilhado)**, **0044 um App único na Meta + roteador de WhatsApp**, **0045 só se extrai serviço com dono externo (não dividir por domínio)**, **0046 rename SMSMais (carve-outs de runtime — ler antes de qualquer sed de nome)**, **0047 posse de conversa (fila da unidade × lista pessoal, claim, encaminhar/transferir)**, **0048 aba "Todas" destravada + histórico completo do paciente na thread (leitura sem trava de escopo; ações seguem por posse)**, **0050 motor do robô de atendimento na Messages API (loop de tool-use no .NET; resposta só pela ferramenta terminal)**, **0051 treinamento do robô (crítica do atendente → adversários → correção aplicada com desfazer + simulação; agente só mexe em treino e condição)**, **0052 solicitação de regulação como entidade própria (fila de pré-regulação, agente regulador, máquina de estados com trilha append-only; o papel faz parte da transição)**, **0055 catálogo canônico de procedimentos (SISREG+SER+SERNIT sob um procedimento só; busca híbrida lexical+vetorial no Postgres; pareamento entre sistemas sugerido pela máquina e confirmado por pessoa; catálogo PLANO, sem hierarquia)**, **0056 escopo de exame por unidade (o que a unidade executa, a flag de worklist e o aparelho de destino saem do cadastro global e viram par tipo×unidade)**, **0058 estratégias de fila (simulador determinístico em .NET; agente só escolhe parâmetros livres e responde pela ferramenta terminal; trava é contrato; rodada append-only; nada escreve no SISREG — "aplicada" é anotação humana)**, **0059 atendimento humano de confirmação (posse por solicitação com Assumir explícito; 4 abas derivadas; humano entrou = automático não tenta mais; cancelar é local na fase 1 — SISREG pelo navegador, extensão concilia; fase 2 = senha em memória por sessão + escrita com allowlist após spike com OK; Mensageria = gestão dos envios; custos são módulo próprio)**, **0061 modos de assinatura por médico (Desktop/Nuvem VIDaaS/Sem certificado; escolhido pelo admin no cadastro; carimbado ≠ assinado) + selo de verificação do laudo por QR Code (página pública + download)**, **0062 campanhas (local e endereço próprios sobre a unidade do SISREG; entrega direta sem conferência cadastral com link que só confirma presença; envio pelo botão + alcance + reenvio a quem não respondeu)**) |
