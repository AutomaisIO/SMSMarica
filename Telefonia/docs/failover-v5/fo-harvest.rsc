...
:foreach a in=[/ip arp find interface=bridge-transparente] do={
  ...
  :if ([:len $mac] > 0) do={
    :if ([:typeof [:find $ip "10.3.74."]] != "nil") do={
      :if ((($ip != "10.3.74.1") and ($ip != "10.3.74.254")) and ([:len [/ip dhcp-server lease find address=$ip]] = 0)) do={
        :do {
          /ip dhcp-server lease add server=FAILOVER-dhcp address=$ip mac-address=$mac comment="harvest 020926"
          :set n ($n + 1)
        } on-error={ :set j ($j + 1) }
      }
    }
