# FO-tick v5 (Motor B) - BOQUEIRAO id=7
...
:foreach r in=[/ip firewall nat find comment~"FO dns udp"] do={
...
# --- transicao ---
:if ($want != $emFO) do={
  :foreach c in=[/ip firewall connection find src-address~"10.1.108"] do={ :do { /ip firewall connection remove $c } on-error={} }
