# =====================================================================
# FASE SITE - id 7 - CEO Boqueirao (Odonto)
# MK-BORDA-CEO-BOQUEIRAO (hEX RB750Gr3, MAC etiqueta 04f41cd8dc05)
# Aplicado remoto em 2026-07-29 via automais-vpn (10.35.0.30)
#
# Dados L3 levantados no site (dhcp-client TEMP na bridge-transparente):
#   LAN da unidade .......: 10.1.108.0/24
#   Gateway real .........: 10.1.108.1        (ping 1-3ms)
#   IP do MK na LAN ......: 10.1.108.254      (verificado livre)
#   DHCP real (central) ..: 10.1.201.254      (mesmo servidor da Regulacao)
#   DNS reais ............: 10.135.16.119 , 10.135.16.17
#   Dominio ..............: pmm.local         (padrao PMM, mesmo servidor DHCP)
# =====================================================================

# --- 1. Identidade do MK na LAN da unidade ---------------------------
/ip address add address=10.1.108.254/24 interface=bridge-transparente comment="MK LAN id=7"

# --- 2. Regras de FAILOVER (criadas DESABILITADAS; o script liga/desliga)
/ip address add address=10.1.108.1/32 interface=bridge-transparente comment="FAILOVER gw takeover" disabled=yes
/ip firewall nat add chain=srcnat action=masquerade src-address=10.1.108.0/24 out-interface=ether2 comment="FAILOVER nat clientes connect" disabled=yes
/ip firewall nat add chain=dstnat action=redirect protocol=udp dst-port=53 src-address=10.1.108.0/24 in-interface=bridge-transparente to-ports=53 comment="FAILOVER dns udp" disabled=yes
/ip firewall nat add chain=dstnat action=redirect protocol=tcp dst-port=53 src-address=10.1.108.0/24 in-interface=bridge-transparente to-ports=53 comment="FAILOVER dns tcp" disabled=yes
...
/ip pool add name=pool-failover-lan ranges=10.1.108.10-10.1.108.200
/ip dhcp-server network add address=10.1.108.0/24 gateway=10.1.108.1 dns-server=10.135.16.119,10.135.16.17 domain=pmm.local comment="FAILOVER rede (copia fiel DHCP real)"
