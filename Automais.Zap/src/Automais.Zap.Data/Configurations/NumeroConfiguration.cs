...
public sealed class NumeroConfiguration : IEntityTypeConfiguration<Numero>
{
    public void Configure(EntityTypeBuilder<Numero> b)
    {
        ...
        b.Property(x => x.WabaId).HasColumnName("waba_id").HasMaxLength(60);
        b.Property(x => x.DisplayPhoneNumber).HasColumnName("display_phone_number").HasMaxLength(40);
        b.Property(x => x.Rotulo).HasColumnName("rotulo").HasMaxLength(120);
        ...
        // O roteamento inteiro depende deste índice ser único: dois destinos reivindicando
        // o mesmo phone_number_id seria mensagem de um município caindo em outro.
        b.HasIndex(x => x.PhoneNumberId).IsUnique().HasDatabaseName("ux_numero_phone_number_id");

        b.HasOne(x => x.Destino)
            .WithMany(d => d.Numeros)
