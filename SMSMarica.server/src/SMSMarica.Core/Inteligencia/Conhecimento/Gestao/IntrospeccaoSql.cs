...
///
/// Colunas de <see cref="Colunas"/>: <c>esquema, tabela, coluna, tipo, tamanho, anulavel, ordem</c>.
...
public static class IntrospeccaoSql
{
    public static string Colunas(DialetoSql dialeto) => dialeto switch
    {
        ...
        // Oracle: filtra o owner via ALL_TAB_COLUMNS (o esquema é passado no filtro pela chamada).
        DialetoSql.Oracle => """
            SELECT owner AS esquema, table_name AS tabela, column_name AS coluna,
                   data_type AS tipo, data_length AS tamanho, nullable AS anulavel,
                   column_id AS ordem
              FROM all_tab_columns
             WHERE owner NOT IN ('SYS','SYSTEM','XDB','CTXSYS','MDSYS','ORDSYS')
