...
(gen_random_uuid(), 4, '1', 10, 'DISTRIBUIÇÃO DOS ATENDIMENTOS DE U&E SEGUNDO MOTIVO DOS ATENDIMENTOS', 'Possibilita a análise da relação entre o perfil de morbidade da demanda e a necessidade de complexidade da oferta de serviços', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos, segundo o MOTIVO.', NULL, NULL, NULL, NULL, NULL, 4, 'atendimentos', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT NVL(LTRIM(m.ds_mot_atendimento, ''. ''), ''NAO INFORMADO'') AS rotulo,
       COUNT(*) AS quantidade
  FROM infosaude.baa b
...
(gen_random_uuid(), 4, '2', 20, 'DISTRIBUIÇÃO DOS ATENDIMENTOS DE U&E SEGUNDO CLASSIFICAÇÃO DE RISCO', 'Possibilita a análise da relação entre o perfil de morbidade da demanda e a necessidade de complexidade da oferta de serviços', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos, segundo a CLASSIFICAÇÃO DE RISCO.', NULL, NULL, NULL, NULL, NULL, 4, 'atendimentos', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT NVL(UPPER(r.ds_classificacao_risco), ''SEM CLASSIFICACAO DE RISCO'') AS rotulo,
       COUNT(*) AS quantidade
  FROM infosaude.baa b
...
(gen_random_uuid(), 4, '3', 30, 'DISTRIBUIÇÃO DOS ATENDIMENTOS DE U&E SEGUNDO SEXO E FAIXA ETÁRIA', 'Possibilita a análise do perfil demográfico da demanda, relacionando com a cobertura e capacidade de atendimento pela rede atenção primária à saúde segundo o local de origem do paciente (pacientes azuis.', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos, segundo SEXO e FAIXA ETÁRIA. OBS: Se o sistema de prontuário não possuir relatório que apresente os 2 atriburos (sexo e faixa etária), pode enviar os 2 relatórios separados. As faixas etárias adotadas são: 0 a <1 ano; 1 - 4; 5 - 9; 10 -12; 13 - 15; 16 -19; 20-29; 30- 39; 40-49; 50-59; 60 e mais.', NULL, NULL, NULL, NULL, NULL, 4, 'atendimentos', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT base.sexo || '' · '' || DECODE(base.ix,
          1,  ''0 a <1 ano'',
          2,  ''1-4'',
          3,  ''5-9'',
          4,  ''10-12'',
...
(gen_random_uuid(), 4, '4', 40, 'DISTRIBUIÇÃO DOS ATENDIMENTOS SEGUNDO DIAS DA SEMANA E HORÁRIOS DE MAIOR PICO DE ATENDIMENTOS', 'Possibilita analisar a dinâmica dos atendimentos no serviço e eventual necessidade de adequações', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos, segundo DIAS DA SEMANA e HORÁRIOS de maior pico.', NULL, NULL, NULL, NULL, NULL, 4, 'atendimentos', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT DECODE(TRUNC(b.dt_atendimento) - TRUNC(b.dt_atendimento, ''IW''),
...
(gen_random_uuid(), 4, '5', 50, 'DISTRIBUIÇÃO DOS ATENDIMENTOS SEGUNDO ORIGEM DO PACIENTE (DISTRITO SANITÁRIO, BAIRRO, MUNICÍPIO)', 'Possibilita análise da distribuição geográfica dos atendimentos via a vis cobertura assistencial em atenção primária à saúde', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos, segundo o BAIRRO', NULL, NULL, NULL, NULL, NULL, 4, 'atendimentos', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT CASE
...
(gen_random_uuid(), 4, '6', 60, 'PROPORÇÃO DOS ATENDIMENTOS DE U&E DE NÃO RESIDENTES DO MUNICÍPIO DE MARICÁ', 'Análise da sobrecarga da rede municipal por população não residente', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição dos atendimentos de NÃO RESISDENTES do Município de Maricá.', NULL, NULL, NULL, NULL, NULL, 1, '%', NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT COUNT(CASE WHEN p.cd_cidade IS NOT NULL AND p.cd_cidade <> 330270 THEN 1 END) AS numerador,
...
(gen_random_uuid(), 4, '10', 100, 'DISTRIBUIÇÃO DAS INTERNAÇÕES HOSPITALARES POR SEXO E FAIXA ETÁRIA', 'Análise do perfil etário da demanda e suas implicações nos indicadores operacionais de tempo de permanência e mortalidade institucional.', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição das internações, segundo SEXO e FAIXA ETÁRIA. OBS: Se o sistema de prontuário não possuir relatório que apresente os 2 atriburos (sexo e faixa etária), pode enviar os 2 relatórios separados. As faixas etárias adotadas são: 0 a <1 ano; 1 - 4; 5 - 9; 10 -12; 13 - 15; 16 -19; 20-29; 30- 39; 40-49; 50-59; 60 e mais.', NULL, NULL, NULL, NULL, NULL, 4, NULL, NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT s.sexo || '' '' || CHR(183) || '' '' ||
       DECODE(s.ix,  1, ''0 a <1'',
                     2, ''1-4'',
                     3, ''5-9'',
                     4, ''10-12'',
...
(gen_random_uuid(), 4, '11', 110, 'DISTRIBUIÇÃO DAS INTERNAÇÕES POR CID-10 PRINCIPAL DE SAÍDA (ADULTOS; CRIANÇAS E MULHER/GESTANTE)', 'Análise das causas de internação da demanda espontânea; análise do grau de adequação da oferta de leitos e nível de complexidade', 'Envio de relatório do sistema de prontuário eletrônico, que apresente a distribuição das internações por CID-10 de saída.', NULL, NULL, NULL, NULL, NULL, 4, NULL, NULL, 1, (SELECT id FROM smsmarica.ia_fonte WHERE slug = 'salux-hcml' AND excluido_em IS NULL LIMIT 1), 'SELECT CASE WHEN f.cd_cid IS NULL THEN ''SEM CID REGISTRADO''
