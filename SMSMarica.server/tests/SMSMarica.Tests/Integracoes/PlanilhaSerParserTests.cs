...
public class PlanilhaSerParserTests
{
    ...
    // O export da tela de Solicitação quebra "Agendado para" (data - UNIDADE) em até três linhas
    // do arquivo. Medido em 08/08/2026: 100 registros de ALTA viraram 300 linhas.

    private static readonly Dictionary<string, int> Colunas =
        new() { ["id"] = 0, ["data_solicitacao"] = 1, ["agendado_para"] = 2, ["paciente"] = 3 };

    /// <summary>Os fragmentos remontam o texto original da tela. Sem isso, a unidade onde o
    /// paciente foi atendido se perde — e para ALTA ela não vem de mais lugar nenhum.</summary>
    [Fact]
    public void Fragmentos_de_agendado_para_voltam_para_o_registro_anterior()
    {
        var linhas = new List<SerLinhaGrade>
        {
            new() { IdSer = "8107827", AgendadoPara = "05/08/2026" },
        };

        PlanilhaSerParser.AnexarContinuacao(linhas, Colunas, [null, null, "-", null]);
        PlanilhaSerParser.AnexarContinuacao(linhas, Colunas, [null, null, "HOSPITAL X", null]);

        linhas.Should().ContainSingle();
        linhas[0].AgendadoPara.Should().Be("05/08/2026 - HOSPITAL X");
    }

    /// <summary>
    /// Linha com conteúdo em QUALQUER outra coluna não é continuação — é rodapé, totalizador ou
    /// registro que o parser não reconheceu. Grudá-la no anterior seria inventar dado.
    /// </summary>
    [Fact]
    public void Linha_com_outra_coluna_preenchida_nao_e_continuacao()
    {
        var linhas = new List<SerLinhaGrade>
        {
            new() { IdSer = "8107827", AgendadoPara = "05/08/2026" },
        ...
        PlanilhaSerParser.AnexarContinuacao(linhas, Colunas, [null, null, "HOSPITAL X", "JOAO"]);

        linhas[0].AgendadoPara.Should().Be("05/08/2026");
    }

    [Fact]
    public void Continuacao_sem_registro_anterior_e_ignorada()
    {
        var linhas = new List<SerLinhaGrade>();

        var acao = () => PlanilhaSerParser.AnexarContinuacao(linhas, Colunas, [null, null, "-", null]);
